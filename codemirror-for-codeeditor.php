...
$cfc_ver = "0.3";
$cfc_db_ver = "0.3";
$cfc_setting_opt = get_option('cfc_setting_opt');
...
add_action('plugins_loaded', 'cfc_check_db_ver');
...
function cfc_check_db_ver(){
	...
	if (!$current_checkver_stamp || version_compare($current_checkver_stamp, $cfc_db_ver, "!=")) {
		...
		update_option('cfc_checkver_stamp', $cfc_db_ver);
	}
}

// Register the setting panel and hooks
add_action('admin_menu', 'cfc_register_menu_item');

function cfc_register_menu_item() {
	$cfc_page_hook = add_options_page('CodeMirror for CodeEditor', 'CodeMirror for CE', 'manage_options', 'codemirror-for-codeeditor-options', 'cfc_options_panel');
	if ($cfc_page_hook != null) {
		$cfc_page_hook = '-'.$cfc_page_hook;
	}
	add_action('admin_print_scripts'.$cfc_page_hook, 'cfc_load_jscript_for_admin');
	if (get_option('cfc_updated') == "true" && !(isset($_POST['CFC_Setting_Submit']) && $_POST['cfc_hidden_value'] == "true") && !(isset($_POST['CFC_Reset']) && $_POST['cfc_reset'] == "true")) {
		add_action('admin_notices', 'cfc_admin_updated_notice');
	}
}
...
function cfc_load_jscript_for_admin(){
	...
	wp_enqueue_script('rc_admin_js', $cfc_plugin_url.'rc-admin-js.js', false, '1.1');
}
...
function cfc_run_script() {
	global $cfc_plugin_url, $cfc_ver, $cfc_setting_opt, $pagenow;
	if (isset($_GET['file'])) {
...
<?php
	echo "<!-- CodeMirror for CodeEditor Ver.".$cfc_ver." Scripts for toolbar End -->
...
foreach ($theme_list as $val) {
	...
	echo "<!-- CodeMirror for CodeEditor Ver.".$cfc_ver." Scripts for creating toolbar End -->\n";
...
function cfc_additional_style() {
	global $cfc_ver, $cfc_setting_opt;
...
function cfc_options_panel(){
	global $cfc_plugin_url, $cfc_db_ver, $cfc_setting_opt;
	if(!function_exists('current_user_can') || !current_user_can('manage_options')){
		die(__('Cheatin&#8217; uh?'));
	} 
	add_action('in_admin_footer', 'cfc_add_admin_footer');
	...
	<?php _e("CodeMirror for CodeEditor URL:", "cfc_lang") ?> <?php echo $cfc_plugin_url; ?><br />
